#+TITLE: Documentation generation for H2-sym s
...
* Workflow for editing
  ...
  The command generates all the files for a static website and writes
  them in ~Documentation/_build/html~. Some features such as embedding
  youtube videos may not be seen when viewing the documentation from
  file. In such cases, it may be useful to look at a web version using
  for example
  #+BEGIN_SRC sh
...
** Compilation
   ...
   [[https://github.com/BattMoTeam/BattMo/blob/main/Utilities/Various/MLXnotebookExporter.m][here]]. The following [[https://github.com/BattMoTeam/BattMo/blob/main/Documentation/utils/exportMLXnotebooks.m][script]] gives example on how it can be used. I have noticed problems with older Matlab
...
*** link using nbsphinx-link
    ...
    - insert link in index, see example ~runThermalExample.nblink~ in [[https://github.com/BattMoTeam/BattMo/blob/main/Documentation/intermediate.rst?plain=1#16][here]] (line 16)
